...
const formListOptions = ['Form List 1', 'Form List 2', 'Form List 3'];
  ...
  // const createPDF = async (formData) => {
  //   let options = {
  //     html: '<h1>Driving Test Form-3 Submission</h1>' + Object.entries(formData).map(([key, value]) => `<p><strong>${convertCamelCaseToReadable(key)}:</strong> ${value}</p>`).join(''),
  //     fileName: 'Driving Test Form-3 Submission',
  //     directory: 'Documents',
  //   };
  ...
  //       const forms = querySnapshot.docs.map(doc => {
  //         const data = doc.data();
  //         return { id: doc.id, name: data.userName, formData: data.formData, fieldOrder: data.fieldOrder };
  //       });
  //       setForms(forms);
  //     });
  ...
  //   const formDataArray = Object.entries(item.formData);
  ...
  <TouchableOpacity style={styles.selectionBtn} onPress={() => navigation.navigate('FormList3')}>
    ...
    <View style={styles.tableHeader}>
        ...
        <Text style={styles.headerCellMain}>Driving Test Form-3 Submissions</Text>
      </View>
      <View style={styles.tableHeader}>
